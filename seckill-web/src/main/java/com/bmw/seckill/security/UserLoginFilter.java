            ...
            if (session.getAttribute(WebUserUtil.SESSION_WEBUSER_KEY) != null) {
                ...
                if (StringUtils.isNotEmpty(tokenValue)) {//存在token
                    ...
                    if (object != null) {//token的值不为空
                        ...
                        filterChain.doFilter(request, response);
                        return;
                    } else {//session中和token中都没有用户信息：token里无值
                        //返回接口调用方需要登录的错误码，接口调用方开始登录
                        returnJson(response);
                        return;
                    }
                } else {//session中和token中都没有用户信息：不存在token
                    //返回接口调用方需要登录的错误码，接口调用方开始登录
                    returnJson(response);
                    return;
                }
